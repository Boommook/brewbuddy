...
export default function RecipeForm({ mode, recipe }: RecipeFormProps) {
  ...
  const [category, setCategory] = useState<BrewCategory>(
    recipe?.category ?? "MEAD"
  );
  const [meadSubtype, setMeadSubtype] = useState<MeadSubcategory | null>(
    recipe?.meadSubtype ?? null
  );
  const [targetVolume, setTargetVolume] = useState(
    recipe ? String(recipe.targetVolume) : ""
  );
  const [ingredients, setIngredients] = useState<IngredientDTO[]>([]);
  const [rows, setRows] = useState<IngredientLineRow[]>(() =>
    recipe?.ingredients.length
      ? recipe.ingredients.map(ingredientLineRowFromDTO)
      : []
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Catalog is optional for custom ingredient names.
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    if (ingredientsPayload.length === 0) {
      setFormError("Add at least one ingredient line to your recipe.");
      return;
    }
    ...
    const payload = {
      ...
      targetVolumeUnit: recipe?.targetVolumeUnit ?? "GAL",
      ...
      <div>
        <div className="mb-6 flex gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="nunito-sans-regular text-gray-700">
              {isEdit
                ? "Update your recipe details and ingredient lines."
                : "Create a recipe to use for future batches."}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {category === "MEAD" ? (
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setMeadSubtype(
                      e.target.value ? (e.target.value as MeadSubcategory) : null
                    )
                  }
                >
          ...
          <IngredientLinesSection
            title="Ingredients"
            description="Add the ingredients that your recipe requires."
            emptyMessage={
              <>
                Add honey, fruit, water, yeast, or anything else your recipe
                needs. Lines left blank are ignored.
              </>
            }
            rows={sortedRows}
          ...
          <div className="flex flex-wrap gap-3">
            <Tooltip>
              ...
              <TooltipContent side="bottom">
                {name !== "" ? (
                  <p>{isEdit ? "Save recipe" : "Create a new recipe"}</p>
                ) : (
